...
var DEFAULT_BATCH_SIZE = 50,
    ...
    QUERY = "SELECT zoom_level z, tile_column x, (1 << zoom_level) - 1 - tile_row y, tile_data data FROM tiles WHERE z = ? AND x BETWEEN ? AND ? AND y BETWEEN ? AND ? LIMIT ? OFFSET ?";
    ...
    source.getInfo(function(err, info) {
      ...
      _(function(push, next) {
        ...
        return _(function(push, next) {
          return source._db.each(QUERY, zoom, xyz.minX, xyz.maxX, xyz.minY, xyz.maxY, options.batchSize, offset, function(err, tile) {
            push(err, tile);
          }, function(err, count) {
            if (count === 0) {
